...
class Life(object):
    def __init__(self, aGene=None):
        ...
        self.best_score = BEST_SCORE_NONE
        self.v = []

class PSO_TSP:
    def __init__(self, citys, pop_size = 150, max_iter = 3000, w = 0.7, c1 = 1.2, c2 = 1.2):
        self.city = citys
        self.num_citys = len(citys)
        self.pop_size = pop_size#粒子群数量
        self.max_iter = max_iter#最大迭代次数
        self.w = w
        self.c1 = c1
        self.c2 = c2
        self.globle_best_way = None
        self.globle_best_score = -1
        self.particles = []
        self.best_history = []
    ...
    def init_particles(self):
        base = list(range(self.num_citys))
...
def is_end(history, cur_time):
    if cur_time <= 300:
        return 1                 #继续
    elif (history[cur_time - 300]) - (history[cur_time]) <= 0.01:
        return 0                 #停止
    else:
        return 1
...
for gen in range(MAX_GENERATIONS):
    ...
    for j in lives:
        j.score = SCORE_NONE
    if is_end(best_history, gen) == 0:
        break

# 7) 输出结果
final_best_distance = 1.0 / best.score
print(f"经过 {gen} 次迭代，模型收敛，最优解距离为：{final_best_distance:.6f}")
print("遍历城市顺序为：")
